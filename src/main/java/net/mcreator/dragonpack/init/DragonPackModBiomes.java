...
import net.mcreator.dragonpack.world.biome.EdedBiome;
import net.mcreator.dragonpack.world.biome.DrakeForestBiome;
...
public class DragonPackModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, DragonPackMod.MODID);
	public static final RegistryObject<Biome> DRAKE_FOREST = REGISTRY.register("drake_forest", () -> DrakeForestBiome.createBiome());
	public static final RegistryObject<Biome> EDED = REGISTRY.register("eded", () -> EdedBiome.createBiome());

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			DrakeForestBiome.init();
			EdedBiome.init();
		});
	}
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(DrakeForestBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, DRAKE_FOREST.getId()))));
						parameters.add(new Pair<>(EdedBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, EDED.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, DRAKE_FOREST.getId()),
											Blocks.LIME_CANDLE_CAKE.defaultBlockState(), Blocks.DIAMOND_BLOCK.defaultBlockState(),
											Blocks.POINTED_DRIPSTONE.defaultBlockState()));
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, EDED.getId()),
											Blocks.BONE_BLOCK.defaultBlockState(), Blocks.BIG_DRIPLEAF.defaultBlockState(),
